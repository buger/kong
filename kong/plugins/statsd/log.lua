...
local metrics = {
  ...
  status_count = function (scope_name, message, metric_config, logger, conf)
    logger:send_statsd(string_format("%s.request.status.%s", scope_name, message.response.status),
      1, logger.stat_types.counter, metric_config.sample_rate)

    logger:send_statsd(string_format("%s.request.status.%s", scope_name, "total"),
      1, logger.stat_types.counter, metric_config.sample_rate)
  end,
  status_count_per_user = function (scope_name, message, metric_config, logger, conf)
    ...
    if consumer_id then
      logger:send_statsd(string_format("%s.user.%s.request.status.%s", scope_name,
