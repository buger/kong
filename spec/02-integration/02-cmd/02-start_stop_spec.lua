...
describe("kong start/stop #" .. strategy, function()
  ...
  describe("--cleanup-sockets #cleanup", function()
    ...
    local function start(clean)
      local cmd = string.format("start -p %q%s",
                                prefix,
                                clean and " --cleanup-sockets" or "")
    ...
    it("is not enabled by default", function()
      local ok, _, _, stderr = start(false)

      assert(helpers.path.exists(event_sock))

      assert.falsy(ok, "expected `kong start` to fail")
      assert.matches("Address already in use", stderr)
      assert.matches("found dangling unix sockets", stderr)
    end)

    it("removes unix socket files in the prefix directory", function()
      local ok, code, stdout, stderr = start(true)
      assert.truthy(ok, "expected `kong start --cleanup-sockets` to succeed: " .. tostring(code or stderr))
      ...
      assert.matches("removing leftover unix socket", stdout)
      assert.matches(event_sock, stdout)
    end)
